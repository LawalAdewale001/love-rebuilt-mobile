...
function useIncomingCallSocket() {
  ...
  useEffect((): (() => void) => {
    ...
    const register = (socket: ReturnType<typeof getSocket>) => {
      ...
      cleanup = () => {
        socket.off("call:invited", onInvited);
      };
    };
...
function useNotificationNavigation(authReady: boolean) {
  ...
  const navigate = (response: Notifications.NotificationResponse) => {
    ...
    if (data.type === "message" && data.conversationId) {
      ...
      const { queryKeys } =
        require("@/lib/queries") as typeof import("@/lib/queries");
      const { getAuthUser } =
        require("@/lib/auth-store") as typeof import("@/lib/auth-store");
      ...
      if (cachedList?.pages) {
        for (const page of cachedList.pages) {
          const conv = (page.result as any[])?.find(
            (c: any) => c.id === data.conversationId,
          );
          if (conv) {
            const other = (conv.members as any[])?.find(
